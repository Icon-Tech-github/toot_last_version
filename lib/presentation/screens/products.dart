...
class ProductScreen extends StatefulWidget {
  ...
  const ProductScreen({Key? key, this.animationController,this.id,this.endColor,this.startColor,this.depName, this.departments}) : super(key: key);
        ...
        backgroundColor: Colors.transparent,
         body:
           Container(
             ...
             child:
            GetAppBarUi(animationController: productsAnimationController,id: widget.id,startColor: widget.startColor,endColor: widget.endColor,depName: widget.depName,departments: widget.departments,contextUpper: context,)),
      ),
    );
  }
...
class GetAppBarUi extends StatefulWidget {
  const GetAppBarUi({Key? key, this.animationController,this.id,this.startColor,this.endColor,this.depName, this.departments,this.contextUpper}) : super(key: key);
  final AnimationController? animationController;
  final int ? id;
  final  String? startColor;
...
class _GetAppBarUiState extends State<GetAppBarUi> {
  ...
  Widget build(BuildContext context) {
    ...
    BlocProvider<ProductsCubit>(
                ...
                child: ProductsList(contextUpper:widget.contextUpper,animationController:widget.animationController ,startColor: widget.startColor,endColor: widget.endColor,id: widget.id,depName: widget.depName,departments: widget.departments,
...
class ProductsList extends StatefulWidget {
  const ProductsList({Key? key, this.animationController,this.endColor,this.startColor,this.id,this.depName,this.departments,this.mainScreenAnimationController,this.contextUpper}) : super(key: key);
  final AnimationController? animationController;
  final  String? startColor;
  final String ? endColor;
  final int ? id;
      ...
      if (state is ProductsLoading) {
        ...
        return SafeArea(
          child: SmartRefresher(
            ...
            onLoading: (){
              productState.onLoad(DepartmentsCubit.activeId!,context,lang);
            },
            enablePullUp: true,
                          ...
                          return InkWell(
                            onTap: (){
                              //setState(() {
                                ...
                                productState.page=1;
                                productState.onLoad(  widget.departments![index].id!,widget.contextUpper,lang);
                             // });
                            },
                            child: Container(
                              height: size.height * 0.09,
                              ...
                              margin: EdgeInsets.symmetric(horizontal: 5),
                              decoration: BoxDecoration(
                                color:  DepartmentsCubit.activeId! ==   widget.departments![index].id?Color(0xffFFC25C): Colors.white,
                                boxShadow: <BoxShadow>[
                                  BoxShadow(
                                      color: DepartmentsCubit.activeId! ==   widget.departments![index].id? Color(0xffFFC25C).withOpacity(.5):Colors.grey
                                          .withOpacity(0.3),
                                      offset: const Offset(1.1, 4.0),
                                      blurRadius: 8.0),
                                ],
                                border: Border.all(color:  DepartmentsCubit.activeId! ==   widget.departments![index].id?Colors.transparent:Colors.transparent,width: 3),
                                // gradient: LinearGradient(
                                //   colors: <HexColor>[
                                //     HexColor(startColor!),
                                //     HexColor(endColor!),
